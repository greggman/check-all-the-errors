...
class Tester extends EventEmitter {
  ...
  async run(options) {
    // Annoying but url has 2 meanings, a string and a URL object.
    ...
    page.on('response', response => {
      ...
      if (!urlInfo.isStatusSet()) {
        ...
        this.emit('response', {href: currentHref, resourceHref: response.url(), status: response.status()});
      }
    });
    ...
    while (!this.runner.exiting && urls.length) {
      ...
      try {
        ...
        let tryHref = href;
        const redirects = [];
        for (let tryCount = 0; tryCount < maxRedirects; ++tryCount) {
          result = await page.goto(tryHref, {waitUntil: 'networkidle2', timeout});
          status = result.status();
          if (status < 300 || status > 399) {
            break;
          }
          const redirectHref = result.headers().location;
          if (!redirectHref) {
              break;
          }
          redirects.push(redirectHref);
          tryHref = redirectHref;
        }
        this.emit('status', {href, status, ...redirects.length && {redirects}});
      ...
      for (const [href, remoteURLInfo] of remoteURLInfoMap) {
        ...
        try {
          ...
          if (!urlInfo.isStatusSet()) {
            ...
            this.emit('response', {href, resourceHref: href, status: res.status});
          }
        } catch (e) {
          //
        }
